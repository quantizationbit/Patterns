...
// Globals
  ...
  unsigned short stripsize = 1920*2*4;
...
{ 
	     ...
	     while(arg < argc) {
		     ...
		     if(strcmp(argv[arg],"D55")==0) {
		     	 printf("\n Processing D55  data\n");
		     	 D55 = 1;
	
		     } 
		     if(strcmp(argv[arg],"D60")==0) {
		     	 printf("\n Processing D60  data\n");
		     	 D55 = 60;
	
		     } 		     
		     
		     if(strcmp(argv[arg],"Amber")==0) {
		     	 printf("\n Processing Amber  data\n");
		     	 D55 = -99;
  ...
  Line =  (unsigned short *)malloc((4*arraySizeX*sizeof(unsigned short)));
      ...
      printf("V=%g,  L =%g\n", V,L);		
		...
		for (int line = 0;line < arraySizeY;line++)
		{
			...
			// Set APL 0.1
			if(line < arraySizeY/6) APL = 10.0;
			
			// Set APL 1
			if(line >= arraySizeY/6 && line < 2*arraySizeY/6) APL = 20.0;
						
			// Set APL 10
			if(line >= 2*arraySizeY/6 && line < 3*arraySizeY/6) APL = 50.0;
			...
			X = (unsigned short)(16.0 + 0.5 + 4060 * PQ10000_r((3./10000.0) * xD65/yD65));
			Y = (unsigned short)(16.0 + 0.5 + 4060 * PQ10000_r(3./10000.0));
			Z = (unsigned short)(16.0 + 0.5 + 4060 * PQ10000_r((3./10000.0) * (1.0 - xD65 - yD65)/yD65));
			...
			for (unsigned int pixel = 0; pixel < (4*arraySizeX);pixel+=16)
			{
				if(pixel < 8*arraySizeX/2) range = 0.999;
				if(pixel < 7*arraySizeX/2) range = 0.90;				
				if(pixel < 6*arraySizeX/2) range = 0.80;
				if(pixel < 5*arraySizeX/2) range = 0.70;	
				if(pixel < 4*arraySizeX/2) range = 0.65;
				if(pixel < 3*arraySizeX/2) range = 0.50;				
				if(pixel < 2*arraySizeX/2) range = 0.35;
				if(pixel < arraySizeX/2) range = 0.25;				
			...
			Xr = (unsigned short)(16.0 + 0.5 + 4060 * PQ10000_r((APLM/10000.0) * xD65/yD65));
			Yr = (unsigned short)(16.0 + 0.5 + 4060 * PQ10000_r(APLM/10000.0));
			Zr = (unsigned short)(16.0 + 0.5 + 4060 * PQ10000_r((APLM/10000.0) * (1.0 - xD65 - yD65)/yD65));		
				...
				if(pixel < arraySizeX) {
				   ...
				   Line[pixel+20] = Xr << 4;     // R = X
					Line[pixel+21] = Yr << 4;  //G = Y 
					Line[pixel+22] =Zr << 4;   // B = Z
